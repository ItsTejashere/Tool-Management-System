...
public class ToolRepository {
    ...
    public boolean saveTool(Tool tool) {
        ...
        try {
            ...
            // STEP 1: Insert the Tool Blueprint
            String sqlTool = "INSERT INTO tool (tool_code, tool_name, minimum_quantity, total_quantity, storage_location, status, project_id) VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            try (PreparedStatement psTool = con.prepareStatement(sqlTool, java.sql.Statement.RETURN_GENERATED_KEYS)) {
                ...
                psTool.setInt(3, tool.getMinimumQuantity());

                // The total quantity is just the size of the serials list!
                int calculatedTotal = (tool.getSerials() != null) ? tool.getSerials().size() : 0;
                psTool.setInt(4, calculatedTotal);

                psTool.setString(5, tool.getStorageLocation());
                psTool.setString(6, calculatedTotal > 0 ? "AVAILABLE" : "UNAVAILABLE");

                if (tool.getProjectId() != null) {
                    psTool.setInt(7, tool.getProjectId());
                } else {
                    psTool.setNull(7, java.sql.Types.INTEGER);
                }
                ...
                if (newToolId != -1 && tool.getSerials() != null && !tool.getSerials().isEmpty()) {
                    ...
                    try (PreparedStatement psInstance = con.prepareStatement(sqlInstance)) {
                        for (String manualSerial : tool.getSerials()) {
                            ...
                            psInstance.setString(2, manualSerial); // Use the exact string from React
                            psInstance.addBatch();
                        }
    ...
    public boolean updateTool(Tool tool) {
        // Added project_id to the SET clause
        String sql = "UPDATE tool SET tool_code = ?, tool_name = ?, minimum_quantity = ?, total_quantity = ?, storage_location = ?, status = ?, project_id = ? WHERE tool_id = ?";
        try (Connection con = DBConnection.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {

            ps.setString(1, tool.getToolCode());
            ps.setString(2, tool.getToolName());
            ps.setInt(3, tool.getMinimumQuantity());
            ps.setInt(4, tool.getTotalQuantity());
            ps.setString(5, tool.getStorageLocation());
            ps.setString(6, tool.getStatus());

            // Handle the potentially null project ID securely
            if (tool.getProjectId() != null) {
                ps.setInt(7, tool.getProjectId());
            } else {
                ps.setNull(7, java.sql.Types.INTEGER);
            }

            ps.setInt(8, tool.getToolId());
